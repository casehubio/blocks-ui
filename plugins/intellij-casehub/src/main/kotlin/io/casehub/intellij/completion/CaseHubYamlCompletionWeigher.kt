...
class CaseHubYamlCompletionWeigher : CompletionWeigher() {
    ...
    override fun weigh(element: LookupElement, location: CompletionLocation): Comparable<*> {
        ...
        val name = file.name
        if (!name.endsWith(".page.yaml") && !name.endsWith(".dash.yaml") &&
            !name.endsWith(".case.yaml") && !name.endsWith(".swf.yaml") &&
            !name.endsWith(".htn.yaml") && !name.endsWith(".org.yaml")) return 0
