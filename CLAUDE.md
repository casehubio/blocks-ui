...
| `components/commitment-viz/` | Commitment lifecycle visualization — state pills (`commitment-state-pill`), transition badges (`commitment-transition-badge`), range bars (compact/detailed modes), `decorateCommitmentRanges` pure function for feed decoration metadata. Props-driven, decoupled from channel-activity. Types import from blocks-ui-core. |
