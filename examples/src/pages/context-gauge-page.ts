...
@customElement('blocks-example-context-gauge')
export class ContextGaugePage extends LitElement {
  ...
  override render() {
    return html`
      <h2>Context Gauge</h2>
      <p>Inline topbar context usage indicator — 80px progress bar with threshold pulsing. Responds to context-usage pages-events.</p>
      <div class="topbar-mock">
        <span class="topbar-label">Topbar mock:</span>
        <context-gauge id="gauge"></context-gauge>
      </div>
    `;
  }

  override firstUpdated() {
    const gauge = this.shadowRoot!.querySelector('#gauge') as any;
    gauge._visible = true;
    gauge._pct = 67;
    gauge.toggleAttribute('visible', true);
  }
}
